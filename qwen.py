...
필요 패키지:
    ...
    (adapter_dir로 LoRA를 붙이려면 추가로: pip install peft)
"""
...
DEFAULT_MAX_PIXELS = 640 * 28 * 28  # 640 * 28 * 28
...
def load_model_and_processor(
    ...
    - adapter_dir: 지정하면 base 모델 위에 이 경로의 LoRA 어댑터(peft)를 얹어서 반환한다
      (train.py의 --output_dir로 저장된 체크포인트 폴더, 예: checkpoint-4130). 지정 안 하면
      base 모델 그대로 반환 - 기존 호출부(test.py 등에서 별도로 PeftModel.from_pretrained
      하던 방식)와 호환되도록 기본값은 None으로 둠.
    """
    ...
    if adapter_dir:
        ...
        print(f"[qwen.py] Loading LoRA adapter from {adapter_dir} ...")
